...
loading:
	...
	gp = (v3f*)malloc(sizeof(v3f)*n);
	gm = (float*)malloc(sizeof(float)*n);

	for (i = 0; i < n; ++i)
	{
		fread(&gp[i], sizeof(v3f), 1, f);
		fread(&gm[i], sizeof(float), 1, f);
		printf("gpx%d: %f\r\n", i, gp[i].x);
		printf("gpy%d: %f\r\n", i, gp[i].y);
		printf("gpz%d: %f\r\n", i, gp[i].z);
		printf("gm%d: %f\r\n", i, gm[i]);
...
save:
	...
	for (i = 0; i < n; ++i)
	{
		fwrite(&gp[i], sizeof(v3f), 1, f);
		fwrite(&gm[i], sizeof(float), 1, f);
	}
...
starting:
	...
	while (1)
	{
		...
		memset(&v, 0, sizeof(v3f));

		for (j = 0; j < n; ++j)
		{
			v3fsub(&dv, gp[j], a);
			d = mag3f(dv);
			v3fmul(&dv, dv, gm[i] / (d*d*d));
			v3fadd(&v, v, dv);
		}
